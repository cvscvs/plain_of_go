...
func GetAtrticle(r *gin.Context) {
	fmt.Println("arrived here now????")
	param := r.Param("title") //new(request.GetArticleRequest)
	if param == "" {
		r.JSON(http.StatusOK, response.Response{
			Code: 400,
			Msg:  "error param1",
	...
	data, err := repository.GetArticleByTitle(param)
	if err != nil {
		global.Logger.Err(err).Send()
		r.JSON(http.StatusOK, response.Response{
			Code: 500,
	...
	response.RespOk(r, "success", data)
}
